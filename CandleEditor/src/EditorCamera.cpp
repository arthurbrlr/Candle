...
EditorCamera::EditorCamera(double width, double height)
{
	...
	_orthographic = Candle::OrthographicCamera(-_aspectRatio * _zoomLevel, _aspectRatio * _zoomLevel, -_zoomLevel, _zoomLevel);
	_perspective = Candle::PerspectiveCamera(width, height);
...
void EditorCamera::UpdateOrthographicMovement()
{
	...
	if ( Candle::Input::IsKeyPressed(CDL_KEY_UP) ) {
		_cameraTransform.Move({ 0, 0.05, 0 });
		_targetTransform.Move({ 0, 0.05, 0 });
		changed = true;
	}

	if ( Candle::Input::IsKeyPressed(CDL_KEY_DOWN) ) {
		_cameraTransform.Move({ 0, -0.05, 0 });
		_targetTransform.Move({ 0, -0.05, 0 });
		changed = true;
	}

	if ( Candle::Input::IsKeyPressed(CDL_KEY_LEFT) ) {
		_cameraTransform.Move({ -0.05, 0, 0 });
		_targetTransform.Move({ -0.05, 0, 0 });
		changed = true;
	}

	if ( Candle::Input::IsKeyPressed(CDL_KEY_RIGHT) ) {
		_cameraTransform.Move({ 0.05, 0, 0 });
		_targetTransform.Move({ 0.05, 0, 0 });
		changed = true;
	}
...
void EditorCamera::UpdatePerspectiveMovement()
{
	...
	if ( Candle::Input::IsKeyPressed(CDL_KEY_UP) ) {
		_targetTransform.Move(-5.f * (float)Candle::Time::FixedDeltaTime() * forward);
		changed = true;
	}
	...
	if ( Candle::Input::IsKeyPressed(CDL_KEY_LEFT) ) {
		_targetTransform.Move(-5.f * (float)Candle::Time::FixedDeltaTime() * right);
		changed = true;
	}
	...
	if ( changed || _mouseScrolled || Candle::Input::IsMouseButtonPressed(1) ) {
		UpdateCameraPosition();
		UpdateViewMatrix();
...
void EditorCamera::OnEvent(Candle::Event & event)
{
	Candle::EventDispatcher dispatcher(event);
	dispatcher.Dispatch<Candle::MouseScrolledEvent>(BIND_EVENT_FUNCTION(EditorCamera::OnMouseScrolled));
	dispatcher.Dispatch<Candle::MouseMovedEvent>(BIND_EVENT_FUNCTION(EditorCamera::OnMouseMoved));
	dispatcher.Dispatch<Candle::WindowResizeEvent>(BIND_EVENT_FUNCTION(EditorCamera::OnWindowResize));
}
...
bool EditorCamera::OnMouseScrolled(Candle::MouseScrolledEvent& event)
{
	if ( _useOrthographic ) {
...
bool EditorCamera::OnMouseMoved(Candle::MouseMovedEvent& event)
{

	if ( Candle::Input::IsMouseButtonPressed(1) && !_useOrthographic ) {
...
bool EditorCamera::OnWindowResize(Candle::WindowResizeEvent & event)
{
...
Candle::RawCamera& EditorCamera::GetCamera()
{
	if ( _useOrthographic ) return _orthographic;
	else return _perspective;
}

Candle::OrthographicCamera& EditorCamera::GetOrthographic()
{
	return _orthographic; 
}

Candle::PerspectiveCamera& EditorCamera::GetPerspective()
{
	return _perspective;
}

const Candle::Transform& EditorCamera::GetTargetTransform() const
{
